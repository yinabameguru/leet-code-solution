...
public class ExclusiveTimeOfFunctions {
    public static void main(String[] args) {
        ExclusiveTimeOfFunctions mainClass = new ExclusiveTimeOfFunctions();
...
class Solution {
    public int[] exclusiveTime(int n, List<String> logs) {
        ...
        for (int i = 0; i < logs.size(); i++) {
            ...
            //如果是指令开始，则入栈
            if (isStart(log)) {
                list.add(toEntity(log));
            }
            //如果是指令结束，结算时间，即当前指令增加时间，上一个指令减少时间
            else {
                int[] startEntity = list.removeLast();
                int[] endEntity = toEntity(log);
                //当前指令增加占用时间
                addExclusiveTime(startEntity, endEntity, exclusiveTime);
                //上一个指令减少时间，这里不需要遍历所有前置减少时间，因为当前指令消耗的时间会包括了所有子指令的时间
                reduceExclusiveTime(startEntity, endEntity, list, exclusiveTime);
            }
        }
        return exclusiveTime;
    }

    private int[] toEntity(String log) {
        return new int[] {getId(log), getTimestamp(log)};
    }

    private void reduceExclusiveTime(int[] startEntity, int[] endEntity, LinkedList<int[]> list, int[] exclusiveTime) {
        if (list.isEmpty()) {
            return;
        }
        int expendTime = endEntity[1] - startEntity[1] + 1;
        exclusiveTime[list.getLast()[0]] = exclusiveTime[list.getLast()[0]] - expendTime;
    }

    private void addExclusiveTime(int[] startEntity, int[] endEntity, int[] exclusiveTime) {
        int id = startEntity[0];
        int expendTime = endEntity[1] - startEntity[1] + 1;
        exclusiveTime[id] = exclusiveTime[id] + expendTime;
    }
